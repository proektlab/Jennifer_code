...
for i = 1:c
    ...
    Ds = [Ds;ds];
    pVals(i) = 1-cdfTukey(q(i), 394 - 6, 6);
end;
warning ('off');
qc = [ones(numel(q),1)]*qc;
warning ('on');
ss = 1:k
ps = e./t
disp(' '); 
...
disp('-------------------------------------------------------------------------');
disp(' Comparision      Difference*        SE        q        qc      pValue      Decision');
disp('-------------------------------------------------------------------------');
for ml=1:k*((k-1)/2)
    fprintf('    %d  %d       %11.4f        %6.3f     %4.3f     %4.3f     %4.3f       %s\n', ... 
        CO(ml,1),CO(ml,2),dpx(ml),se(ml),q(ml),qc(ml,:),pVals(ml), Ds(ml,:));
end;
disp('-------------------------------------------------------------------------');
